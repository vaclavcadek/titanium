...
file_name = 'iris.pmml'
keras2pmml(model, file=file_name, **params)
pmml = ti.read_pmml(file_name)
os.unlink(file_name)

probs_keras = model.predict_proba(X_test_scaled)[:, 1]
probs_titanium = pmml.predict_proba(X_test_scaled)[:, 1]

print('\n')
print('Accuracy (Keras): {accuracy}'.format(accuracy=roc_auc_score(y_test, probs_keras)))
print('Accuracy (Titanium): {accuracy}'.format(accuracy=roc_auc_score(y_test, probs_titanium)))
